...
- **Indipendent:** allthe packages, pages etc ae to be 100% indipendent, only api will be required from the desktop app to connect the mobile devices,the back_office flder tohold allthe data for it with no sharing of codes wih the other folers in root
...
- **Offline First:**
The data or server is the main exe coputer where the api is, the back office stored data temporalily on the mongodb uri, then when the server is backonline,,, all the data is synced to the server, but other recrds are kept permanentlyin the uri, if it is a must
