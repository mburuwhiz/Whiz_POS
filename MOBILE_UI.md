...
- **Purpose:** Secure access to the application and initial connection setup.
- **Connection Mode:**
  - The application connects via an API.
  - **Unique Connection:** Each user/device must be configured with a unique API Key (or connection token) to establish the link with the backend/desktop system.
- **Authentication & Persistence:**
  - **Persistent Login:** The session is persistent. Once logged in, the user remains authenticated.
  - **Session Access:** To resume the active session, the user can authenticate via the device's native **Screen Lock** (Biometrics, Pattern) or a specific **PIN**, instead of entering full credentials every time.
- **Components:**
  - API Key input field (for initial setup).
  - User selection dropdown (if applicable after connection).
  - Unlock prompt (Screen Lock/PIN) for returning users.
...
- Settings
- Sync Status
- Logout (Ends the persistent session)
